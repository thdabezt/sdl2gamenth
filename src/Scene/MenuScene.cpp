...
void MenuScene::render() {
    ...
    if (backgroundTexture)
        SDL_RenderCopy(Game::renderer, backgroundTexture, NULL,
                       &backgroundRect);
    if (titleTexture)
        SDL_RenderCopy(Game::renderer, titleTexture, NULL, &titleRect);
    if (playButtonTexture)
        SDL_RenderCopy(Game::renderer, playButtonTexture, NULL,
                       &playButtonRect);
    if (nameBoxTexture)
...
>>>>>>> 48aebd591664aaebcc837f2de6b6a7394e56c0f2
        ...
        if (nameBoxTexture) {
            ...
            SDL_RenderCopy(Game::renderer, nameBoxTexture, NULL,
                           &currentSlotRect);
        ...
        if (saveSlotFont) {
            std::string textStr =
                currentSlot.isNewGameOption
                    ...
                    : (actualIndex == selectedSaveSlotIndex
                           ? (currentSlot.playerName + " Lvl " +
                              std::to_string(currentSlot.level))
            ...
            SDL_Surface* surface = TTF_RenderText_Blended(
                saveSlotFont, textStr.c_str(), colorToUse);
            if (surface) {
                ...
                if (textTexture) {
                    ...
                    if (Game::renderer)
                        SDL_GetRendererOutputSize(Game::renderer, &w, &h);
                    float scaleX = static_cast<float>(w) / referenceWidth;
                    float scaleY = static_cast<float>(h) / referenceHeight;
                    float scaleFactor =
                        std::max(std::min(scaleX, scaleY), 0.8f);
                    float saveSlotTextScale =
                        std::max(scaleFactor * 0.9f, 0.72f);

                    SDL_Rect textDestRect;
                    textDestRect.w =
                        static_cast<int>(surface->w * saveSlotTextScale);
                    textDestRect.h =
                        static_cast<int>(surface->h * saveSlotTextScale);
                    textDestRect.x = currentSlotRect.x +
                                     (currentSlotRect.w - textDestRect.w) / 2;
                    textDestRect.y = currentSlotRect.y +
                                     (currentSlotRect.h - textDestRect.h) / 2;

                    SDL_RenderCopy(Game::renderer, textTexture, NULL,
                                   &textDestRect);
                    SDL_DestroyTexture(textTexture);
                } else {
                    std::cerr << "Failed to create texture for save slot text: "
                              << textStr << std::endl;
                }
                SDL_FreeSurface(surface);
            } else {
                std::cerr << "Failed to render text surface for save slot: "
                          << textStr << std::endl;
            }
        }
    }
...
<<<<<<< HEAD
    bool needsScrollbar =
        static_cast<int>(saveSlots.size()) > visibleSlotsCount;
=======
...
>>>>>>> 48aebd591664aaebcc837f2de6b6a7394e56c0f2
    if (needsScrollbar) {
        ...
        if (slideButtonTexture) {
            ...
            SDL_RenderCopy(Game::renderer, slideButtonTexture, NULL,
                           &scrollbarButtonRect);
    ...
    if (isEditingName && selectedSaveSlotIndex == 0) {
        ...
        if (ticks % 1000 < 500) {
            ...
            float textScale = std::max(scaleFactor * 0.8f, 0.64f);

            int cursorW = std::max(1, static_cast<int>(2.0f * textScale));
            int cursorPadding = std::max(1, static_cast<int>(2.0f * textScale));
            int cursorH =
                nameTextRect.h > 0
                    ? nameTextRect.h
                    : std::max(1, nameInputBoxRect.h - 2 * cursorPadding);

            int cursorX;
            if (nameTextRect.w > 0) {
                cursorX = nameTextRect.x + nameTextRect.w + cursorPadding;
            } else {
                cursorX = nameInputBoxRect.x + cursorPadding * 2;
            }

            int cursorY =
                (nameTextRect.h > 0)
                    ? nameTextRect.y
                    : (nameInputBoxRect.y + (nameInputBoxRect.h - cursorH) / 2);

            cursorX =
                std::min(cursorX, nameInputBoxRect.x + nameInputBoxRect.w -
                                      cursorW - cursorPadding);
    ...
    if (bgmIconTex)
        SDL_RenderCopy(Game::renderer, bgmIconTex, NULL, &bgmIconRect);
    if (sfxIconTex)
        SDL_RenderCopy(Game::renderer, sfxIconTex, NULL, &sfxIconRect);
    if (sliderTrackTexture) {
        SDL_RenderCopy(Game::renderer, sliderTrackTexture, NULL,
                       &bgmSliderTrackRect);
        SDL_RenderCopy(Game::renderer, sliderTrackTexture, NULL,
                       &sfxSliderTrackRect);
    }
    ...
    if (sliderButtonTexture) {
        SDL_Color bgmTint = isDraggingBgmSlider ? SDL_Color{200, 200, 200, 255}
                                                : SDL_Color{255, 255, 255, 255};
        SDL_SetTextureColorMod(sliderButtonTexture, bgmTint.r, bgmTint.g,
                               bgmTint.b);
        SDL_RenderCopy(Game::renderer, sliderButtonTexture, NULL,
                       &bgmSliderButtonRect);

        SDL_Color sfxTint = isDraggingSfxSlider ? SDL_Color{200, 200, 200, 255}
                                                : SDL_Color{255, 255, 255, 255};
        SDL_SetTextureColorMod(sliderButtonTexture, sfxTint.r, sfxTint.g,
                               sfxTint.b);
        SDL_RenderCopy(Game::renderer, sliderButtonTexture, NULL,
                       &sfxSliderButtonRect);
    ...
    if (fullscreenTextTex) {
        SDL_RenderCopy(Game::renderer, fullscreenTextTex, NULL,
                       &fullscreenTextRect);
    }
